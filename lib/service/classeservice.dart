...
Future updateClasse(Classe classe)async
{
  ...
  Response response = await http.put(
    Uri.parse("http://10.0.2.2:8081/class/update"),
    headers: {
      "Content-type": "Application/json",
    },
    body: jsonEncode(<String, dynamic>{
...
Future<List<Classe>> getClassesByMatiereQuery(int id) async {
  Response response = await http.get(
    Uri.parse("http://10.0.2.2:8081/classe/getByMatiereId"),
  );
...
Future addMatiereToClasse(int classeId, int matiereId) async {
  try {
    Response response = await http.post(
      Uri.parse("http://10.0.2.2:8081/class/addMatiereToClasse"),
      headers: {
...
Future<List<Matiere>> getMatieresByClassId(int classId) async {
  try {
    Response response = await http.get(
      Uri.parse("http://10.0.2.2:8081/class/getMatieresByClassId/$classId"),
    );
